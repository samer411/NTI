...
io.on('connection',async(socket)=>{
    ...
    socket.on('register',async(userName)=>{
        ...
        await User.findOneAndUpdate(
            {userName},{userName,socketId:socket.id,isConnected:true},
            {upsert:true}
        )
        socket.name = userName
        const queue = await Message.find({messageReceiver:userName,isSent:false})
        queue.forEach(m=>{
    ...
    socket.on('roomMessage',async({room,messageBody})=>{
        console.log(room,messageBody)
